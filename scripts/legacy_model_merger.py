...
python scripts/legacy_model_merger.py merge \
    --backend fsdp \
    --local_dir /data/tct/ActivePerception/checkpoints/verl_agent/grpo_qwen3vl-unified-tasks-01_18/global_step_150/actor \
    --target_dir /data/tct/models/RL-only/Qwen3-VL-2B-unified-tasks-rl_only
```
...
def _maybe_prepare_internvl_local_dir(local_dir: str):
    if "internvl" not in local_dir.lower():
        return
    ...
    conversation_path = local_path / "conversation.py"
    if conversation_path.exists():
        return

    init_dir = Path("/data/tct/models/Init/InternVL3_5-2B-Git")
    src_files = [
        "conversation.py",
    ...
    missing = [name for name in src_files if not (init_dir / name).exists()]
    if missing:
        raise FileNotFoundError(f"InternVL init dir missing files: {missing} (dir={init_dir})")

    for name in ["conversation.py", "modeling_intern_vit.py", "modeling_internvl_chat.py"]:
        shutil.copy2(init_dir / name, local_path / name)

    for name in ["config.json", "tokenizer_config.json"]:
        shutil.copy2(init_dir / name, local_path / name)

    print(f"Prepared InternVL local_dir by copying missing files into: {local_dir}")
...
class BaseModelMerger(ABC):
    def __init__(self, config: ModelMergerConfig):
        ...
        print(f"{self.hf_model_config_path=}")

    def get_transformers_auto_model_class(self):
        if "ForTokenClassification" in self.model_config.architectures[0]:
            return AutoModelForTokenClassification
        elif "ForCausalLM" in self.model_config.architectures[0] or "InternVL" in self.model_config.architectures[0]:
...
def main():
    ...
    args.local_dir = _normalize_posix_path(args.local_dir)
    if args.hf_model_path:
        args.hf_model_path = _normalize_posix_path(args.hf_model_path)

    _maybe_prepare_internvl_local_dir(args.local_dir)
    ...
    _maybe_prepare_internvl_local_dir(args.target_dir)
