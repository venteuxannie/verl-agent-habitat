...
class RayPPOTrainer:
    ...
    def _convert_to_json_serializable(self, data):
        ...
        if isinstance(data, np.ndarray):
            ...
            return [self._convert_to_json_serializable(item) for item in data]
        elif hasattr(data, 'item'):  # Handle scalar numpy types
            return data.item()
        else:
            return data

    def _dump_generations(self, inputs, outputs, scores, reward_extra_infos_dict, dump_path, batch=None):
        """Dump rollout/validation samples as JSONL with enhanced GRPO information."""
        ...
        base_data = {
            "input": inputs,
            "output": outputs,
            "score": self._convert_to_json_serializable(scores),
            "step": [self.global_steps] * n,
        }

        # Add reward extra infos
        for k, v in reward_extra_infos_dict.items():
            if len(v) == n:
                base_data[k] = self._convert_to_json_serializable(v)

        # Add GRPO-specific information if batch is provided
        if batch is not None:
            # Add advantages and returns for GRPO analysis
            if "advantages" in batch.batch:
                advantages = batch.batch["advantages"].sum(-1).cpu().tolist()
                base_data["advantages"] = advantages
            
            if "returns" in batch.batch:
                returns = batch.batch["returns"].sum(-1).cpu().tolist()
                base_data["returns"] = returns
            
            # Add token-level rewards for detailed analysis
            if "token_level_rewards" in batch.batch:
                token_rewards = batch.batch["token_level_rewards"].sum(-1).cpu().tolist()
                base_data["token_level_rewards"] = token_rewards
            
            # Add trajectory information for multi-turn scenarios
            if "uid" in batch.non_tensor_batch:
                base_data["uid"] = self._convert_to_json_serializable(batch.non_tensor_batch["uid"])
            
            if "traj_uid" in batch.non_tensor_batch:
                base_data["traj_uid"] = self._convert_to_json_serializable(batch.non_tensor_batch["traj_uid"])
            
            # Add action validity information
            if "is_action_valid" in batch.non_tensor_batch:
                base_data["is_action_valid"] = self._convert_to_json_serializable(batch.non_tensor_batch["is_action_valid"])
            
            # Add data source information
            if "data_source" in batch.non_tensor_batch:
                base_data["data_source"] = self._convert_to_json_serializable(batch.non_tensor_batch["data_source"])

        with open(filename, "w") as f:
            for i in range(n):
                entry = {k: v[i] for k, v in base_data.items()}
                # Ensure all data is JSON serializable before writing
                entry = self._convert_to_json_serializable(entry)
                try:
                    f.write(json.dumps(entry, ensure_ascii=False) + "\n")
                except TypeError as e:
                    print(f"JSON serialization error for entry {i}: {e}")
                    print(f"Entry keys: {list(entry.keys())}")
                    print(f"Entry types: {[(k, type(v)) for k, v in entry.items()]}")
                    # Try to identify problematic values
                    for k, v in entry.items():
                        try:
                            json.dumps(v)
                        except TypeError as ve:
                            print(f"Problematic field '{k}': {ve}, value: {v}, type: {type(v)}")
                    raise
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if self.config.algorithm.adv_estimator == AdvantageEstimator.GRPO:
                            print(f"GRPO Training - Batch size: {len(batch)}, "
                                  f"Token-level rewards shape: {batch.batch['token_level_scores'].shape}, "
                                  f"Response mask shape: {batch.batch['response_mask'].shape}")
                            if "uid" in batch.non_tensor_batch:
                                unique_uids = len(set(batch.non_tensor_batch["uid"]))
                                print(f"GRPO Training - Unique UIDs: {unique_uids}")
                            if "traj_uid" in batch.non_tensor_batch:
                                unique_traj_uids = len(set(batch.non_tensor_batch["traj_uid"]))
                                print(f"GRPO Training - Unique trajectory UIDs: {unique_traj_uids}")
                    ...
                    rollout_log_freq = self.config.trainer.get("rollout_log_freq", 1)  # Default to log every step
                    if rollout_data_dir and (is_last_step or self.global_steps % rollout_log_freq == 0):
                        with _timer("dump_rollout_generations", timing_raw):
                            ...
                            self._dump_generations(
                                ...
                                batch=batch,  # Pass batch for enhanced GRPO information
                            )
