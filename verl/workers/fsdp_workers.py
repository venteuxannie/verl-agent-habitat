...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        with init_context(), warnings.catch_warnings():
            ...
            # InternVL specific: set use_flash_attn in config
            if re.match("internvl", actor_model_config.model_type, re.IGNORECASE):
                # Set use_flash_attn in the config, not as a from_pretrained parameter
                actor_model_config.use_flash_attn = True
            
            actor_module = actor_module_class.from_pretrained(**model_init_kwargs)
            
            # InternVL specific: set img_context_token_id and freeze vision encoder
            use_orig_params = fsdp_config.get("use_orig_params", False)
            frozen_vision_tower = fsdp_config.get("frozen_vision_tower", False)
            
            if re.match("internvl", actor_module.config.model_type, re.IGNORECASE):
                # Set img_context_token_id for InternVL
                actor_module.img_context_token_id = self.tokenizer.convert_tokens_to_ids(
                    self.tokenizer.context_image_token
                )
                # Freeze vision encoder parameters to avoid training issues
                # InternVL can have different attribute names for vision encoder
                vision_attr = None
                if hasattr(actor_module, 'vision_model'):
                    vision_attr = actor_module.vision_model
                elif hasattr(actor_module, 'vision_encoder'):
                    vision_attr = actor_module.vision_encoder
                elif hasattr(actor_module, 'img_encoder'):
                    vision_attr = actor_module.img_encoder
                
                if vision_attr is not None:
                    for param in vision_attr.parameters():
                        param.requires_grad = False
                    frozen_vision_tower = True
                    print(f"InternVL: frozen vision tower and set img_context_token_id to {actor_module.img_context_token_id}")
                else:
                    # Fallback: freeze all parameters with 'vision' in their name
                    for name, param in actor_module.named_parameters():
                        if 'vision' in name.lower():
                            param.requires_grad = False
                    frozen_vision_tower = True
                    print(f"InternVL: frozen vision parameters by name matching and set img_context_token_id to {actor_module.img_context_token_id}")
            
            # Additional handling for other models with vision towers
            if frozen_vision_tower:
                if re.match("gemma3", actor_module.config.model_type, re.IGNORECASE):
                    for name, param in actor_module.named_parameters():
                        if "vision_tower" in name:
                            param.requires_grad = False
                # When vision tower is frozen, use_orig_params must be True
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        meta_info = {
            "eos_token_id": (self.generation_config.eos_token_id 
                           ...
                           else self.tokenizer.eos_token_id) if self.generation_config is not None else self.tokenizer.eos_token_id,
            "pad_token_id": (self.generation_config.pad_token_id 
                           if getattr(self.generation_config, "pad_token_id", None) is not None 
                           else self.tokenizer.pad_token_id) if self.generation_config is not None else self.tokenizer.pad_token_id,
        }
