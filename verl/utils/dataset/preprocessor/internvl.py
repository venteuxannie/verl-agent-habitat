...
class InternVLPreprocessor(BasicPreprocessor):
    ...
    def process_image(self, image, **kwargs):
        """Process image from various formats (PIL, URL, path, base64)."""
    ...
    def process_video(self, video, **kwargs):
        ...
        Args:
            video: Dict with video information
            **kwargs: Additional parameters (nframes, fps, fps_min_frames, fps_max_frames)
    ...
    def process_audio(self, audio, **kwargs):
        """InternVL does not support audio."""
        raise ValueError("InternVL does not support audio")

    def __call__(self, messages, row_dict):
        """Process messages and multi-modal data for InternVL.
        
        Args:
            messages: List of message dicts with role and content
            row_dict: Dict containing multi-modal data
            
        Returns:
            Tuple of (row_dict, model_inputs, input_ids, attention_mask, raw_prompt)
        """
        ...
        # Convert <image> to <IMG_CONTEXT> for older InternVL versions
        ...
        # Adjust max_patches based on number of images
