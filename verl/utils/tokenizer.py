...
def hf_tokenizer(name_or_path, correct_pad_token=True, correct_gemma2=True, **kwargs):
    ...
    # Add InternVL specific token configuration
    ...
    if re.match("internvl", config.model_type, re.IGNORECASE):
        ...
        # Ensure eos_token_id is set (InternVL uses Qwen2 tokenizer which has <|im_end|> as EOS)
        if tokenizer.eos_token_id is None:
            # InternVL typically uses <|im_end|> (token id 151645 for Qwen2)
            if hasattr(tokenizer, 'im_end_id'):
                tokenizer.eos_token_id = tokenizer.im_end_id
            elif '<|im_end|>' in tokenizer.get_vocab():
                tokenizer.eos_token_id = tokenizer.convert_tokens_to_ids('<|im_end|>')
                tokenizer.eos_token = '<|im_end|>'
            else:
                # Fallback: use the config's eos_token_id
                if hasattr(config, 'llm_config') and hasattr(config.llm_config, 'eos_token_id'):
                    tokenizer.eos_token_id = config.llm_config.eos_token_id
                elif hasattr(config, 'eos_token_id'):
                    tokenizer.eos_token_id = config.eos_token_id
        
        print(f"InternVL tokenizer initialized. context_image_token_id: {tokenizer.context_image_token_id}, eos_token_id: {tokenizer.eos_token_id}")
...
def hf_processor(name_or_path, **kwargs):
    ...
    try:
        # Special handling for InternVL processor
        if re.match("internvl", config.model_type, re.IGNORECASE):
            print("Initializing InternVLProcessor")
            ...
            print("InternVL processor initialized successfully")
        else:
            # Use default AutoProcessor for other models (e.g., Qwen-VL)
