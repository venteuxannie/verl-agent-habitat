...
def get_fsdp_wrap_policy(module, config=None, is_lora=False):
    ...
    # Special handling for InternVL: filter out specific decoder layers to avoid wrapping them separately
    if re.match("internvl", module.__class__.__name__, re.IGNORECASE):
        ...
        for mod in default_transformer_cls_names_to_wrap:
            # Filter out LlamaDecoderLayer and InternLM2DecoderLayer as they are internal to the LLM backbone
        ...
        print(f"InternVL FSDP wrap policy: filtered layers to {default_transformer_cls_names_to_wrap}")
    elif re.match("gemma3", module.__class__.__name__, re.IGNORECASE):
        # Also handle Gemma3 if needed
        update_cls_names_to_wrap = []
        for mod in default_transformer_cls_names_to_wrap:
