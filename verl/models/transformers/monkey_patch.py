...
def patch_vlm_for_ulysses_input_slicing(model_class: type, slice_position_ids=False):
    ...
    Args:
        ...
        slice_position_ids: Whether to also slice position_ids (needed for some models like InternVL)
    """
...
def apply_monkey_patch(
    ...
    try:
        ...
        if hasattr(model.config, "text_config"):
            ...
            # For InternVL which uses llm_config
            num_attention_heads, num_key_value_heads = (
    ...
    if model.config.model_type == "qwen2_5_vl":
        ...
        # InternVL uses different LLM backends (Qwen2, Qwen3, etc.)
        # We need to patch the specific LLM model being used
        if ulysses_sp_size > 1:
            # Check which LLM backend is being used and patch accordingly
            ...
            print("Monkey patch InternVL LLM backend for Ulysses SP input slicing")
