...
class HabitatEnvironmentManager(EnvironmentManagerBase):
    ...
    def reset(self):
        ...
        prompt = self.build_text_obs(infos)
        observations = {'text': prompt, 'image': obs, 'anchor': None}
        
        return observations, infos
    
    def step(self, text_actions: List[str]):
        # projection_f now returns: pred_task_types, pred_task_prompts, action_indices, valids
        pred_task_types, pred_task_prompts, actions, valids = self.projection_f(text_actions, env_name="habitat")
        
        # Pass pred_task_types and pred_task_prompts to envs.step for task type verification and reward calculation
        next_obs, rewards, dones, infos = self.envs.step(pred_task_types, pred_task_prompts, actions, valids)
        next_prompt = self.build_text_obs(infos)
        next_observations = {
            'text': next_prompt,
            'image': next_obs,
            'anchor': None  # For GiGPO only
        }
        # add action_valid, pred_task_type, and pred_task_prompt to infos
        for i, info in enumerate(infos):
            info['is_action_valid'] = to_numpy(valids[i])
            info['pred_task_type'] = pred_task_types[i]
            info['pred_task_prompt'] = pred_task_prompts[i]
    ...
    def build_text_obs(self, infos: List[Dict]) -> List[str]:
        ...
        Uses a unified prompt template that allows the model to infer the task type.
        
        The task_description is now generated by the VLM service and stored in info,
        making it more natural and flexible.
        """
        postprocess_text_obs = []
        for i in range(len(infos)):
            # Get task_description from info (generated by VLM service in habitat_envs.py)
            # This is a natural language description that the model needs to understand
            task_description = infos[i].get('task_description', '')
            conf_score = infos[i].get('conf_score') or 0.0
            
            # ===============================================================
            # Legacy: Hard-coded task description generation (commented out)
            # Now task_description is generated by VLM service for more natural descriptions
            # ===============================================================
            # task_type = infos[i]['task_type']
            # task_prompt = infos[i].get('task_prompt', '')
            # 
            # if task_type == "detect":
            #     task_description = TASK_DESC_DETECT
            # elif task_type == "class-detect":
            #     task_description = TASK_DESC_CLASS_DETECT.format(category=task_prompt)
            # elif task_type == "segment":
            #     task_description = TASK_DESC_SEGMENT.format(caption=task_prompt)
            # elif task_type == "3d-box":
            #     task_description = TASK_DESC_3D_BOX.format(caption=task_prompt)
            # elif task_type == "grounding":
            #     task_description = TASK_DESC_GROUNDING.format(caption=task_prompt)
            # else:
            #     raise ValueError(f"Unsupported task type: {task_type}")
            # ===============================================================
            
            # Use the unified template with VLM-generated task_description
...
def make_envs(config):
    ...
    elif "habitat" in config.env.env_name.lower():
        from agent_system.environments.env_package.habitat_sim import build_habitat_envs, habitat_projection
        alpha_conf = getattr(config.env, "alpha_conf", 0.5)
        _envs = build_habitat_envs(dataset_name=config.env.dataset_name, seed=config.env.seed, env_num=config.data.train_batch_size, group_n=group_n, is_train=True, alpha_conf=alpha_conf)
        _val_envs = build_habitat_envs(dataset_name=config.env.dataset_name, seed=config.env.seed + 1000, env_num=config.data.val_batch_size, group_n=1, is_train=False, alpha_conf=alpha_conf)
       
        projection_f = partial(habitat_projection, env_name=config.env.env_name)
        envs = HabitatEnvironmentManager(_envs, projection_f, config)
        val_envs = HabitatEnvironmentManager(_val_envs, projection_f, config)
        return envs, val_envs
