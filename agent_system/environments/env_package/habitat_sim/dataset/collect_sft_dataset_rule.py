...
from agent_system.environments.env_package.habitat_sim.utils.constants import SCENE_DATA_PATH, TEMP_DIR, HM3D_TRAINING_SCENES, REPLICACAD_TRAINING_SCENES
...
def annotate_image(
    ...
    hfov: float = 90.0,
    # 额外的标注数据（用于绘制所有类型的标注）
    bbox_2d: Optional[List[int]] = None,      # 2D边界框，gt是xxyy，需要转换为xyxy
    ...
    if draw_all_annotations:
        ...
        if draw_bbox:
            ...
            # # 如果没有直接的2D bbox，尝试从mask提取
            # if bbox_to_draw is None and final_mask_rle is not None:
            #     bbox_to_draw = extract_bbox_from_mask(final_mask_rle)
            
            # # 如果还没有，尝试从3D bbox投影
            # if bbox_to_draw is None and final_bbox_3d is not None:
            #     bbox_to_draw = extract_bbox_from_3dbox(final_bbox_3d, image_size=image_size, hfov=hfov)
...
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# +++ Ray Actor for parallel scene processing
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

@ray.remote(num_cpus=0.01, num_gpus=0.001)
class SFTSceneCollector:
    """
    Ray Actor for collecting SFT data from a single scene.
    ...
    def collect_scene(
        ...
        """
        Collect SFT data for a single scene.
        
        Args:
            scene_idx: Index of the scene to process
            scene_subfolder: List of scene subfolders
            image_folder: Path to save original images
            annotated_folder: Path to save annotated images
            enable_annotation: Whether to enable image annotation
            save_annotated_only: Whether to save only annotated images
            annotation_options: Annotation configuration options
            base_step_id: Starting step ID for this scene
            
        Returns:
            Dict with scene results and dataset entries
        """
        self._ensure_env()
        ...
        for j in range(self.max_scene_instance):
            try:
                ...
                env_obs = self.env.sim.get_sensor_observations()
                _, _, info["conf_score"], _, _, info["pred"], _ = reward_function(True, info["task_type"], env_obs, info["task_prompt"], 0, True, "move_forward", 0, self.env.sim.get_agent(0).get_state())

                for k in range(self.max_step_length):
                    ...
                    obs, reward, done, info = self.env.step(task_type, task_prompt, action_index, True)
...
def collect_sft_data(
    ...
    output_name: str = "unified_tasks_hm3d_CoT_10",
    save_path: str = SCENE_DATA_PATH,
    scenes_size: int = 10,
    ...
    """
    Collect SFT data using Ray actors for parallel processing.
    """
    # Annotation options
    annotation_options = {
        "draw_text": True,
    ...
    if dataset_name == "HM3D":
        ...
        scene_subfolder = HM3D_TRAINING_SCENES[-4:]
    elif dataset_name == "ReplicaCAD":
    ...
    print(f"Collecting SFT data for {scenes_size} scenes with {num_workers} Ray workers...")
    print(f"Output: {json_file}")
    print()
    
    # Calculate base step IDs for each scene (estimate)
    # Each scene: max_scene_instance * max_step_length steps
    estimated_steps_per_scene = max_scene_instance * max_step_length
    
    # Create Ray actors
    collectors = [
        SFTSceneCollector.remote(
    ...
    pending_tasks = {}  # task_ref -> (scene_idx, scene_name, base_step_id)
    ...
    for i, scene_idx in enumerate(scene_queue[:num_workers]):
        ...
        base_step_id = scene_idx * estimated_steps_per_scene
        ...
        pending_tasks[task_ref] = (scene_idx, scene_subfolder[scene_idx], base_step_id)
    ...
    while pending_tasks:
        ...
        for done_ref in done_refs:
            scene_idx, scene_name, base_step_id = pending_tasks.pop(done_ref)
            ...
            try:
                ...
                if result["status"] == "success":
                    print(f"[{completed}/{scenes_size}] {scene_name}: {result['collected_steps']} samples ✓")
                else:
            ...
            if remaining_scenes:
                ...
                base_step_id = next_scene_idx * estimated_steps_per_scene
                task_ref = collector.collect_scene.remote(
                    next_scene_idx,
                    scene_subfolder,
                    image_folder,
                ...
                pending_tasks[task_ref] = (next_scene_idx, scene_subfolder[next_scene_idx], base_step_id)
    ...
    final_dataset = []
    for new_id, entry in enumerate(all_entries):
        old_image_name = entry["image"]
        new_image_name = f"{format(new_id, '06d')}.png"
        
        # Rename image files if needed
        if old_image_name != new_image_name:
            old_path = os.path.join(image_folder, old_image_name)
            new_path = os.path.join(image_folder, new_image_name)
            if os.path.exists(old_path):
                os.rename(old_path, new_path)
            
            if enable_annotation and not save_annotated_only:
                old_annotated_path = os.path.join(annotated_folder, old_image_name)
                new_annotated_path = os.path.join(annotated_folder, new_image_name)
                if os.path.exists(old_annotated_path):
                    os.rename(old_annotated_path, new_annotated_path)
        
        entry["image"] = new_image_name
    ...
    for result in sorted(results, key=lambda x: x["scene_idx"]):
        if result["status"] == "success":
            ...
            total_samples += result["collected_steps"]
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Collect SFT dataset using Ray for parallel processing")
    parser.add_argument(
        "--dataset",
        type=str,
        default="HM3D",
    ...
    parser.add_argument(
        "--name",
        type=str,
        default="unified_tasks_hm3d_CoT_10",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of Ray workers (default: min(scenes, 10))"
    )
    parser.add_argument(
        "--seed",
        type=int,
...
'''
python collect_sft_dataset_rule.py [options]

Options:
  --dataset HM3D         数据集名称
  --name NAME            输出名称
  --save-path PATH       保存路径
  --scenes N             场景数量 (default: 10)
  --instances N          每个场景的实例数 (default: 300)
  --steps N              每个实例的最大步数 (default: 10)
  --workers N            Ray worker 数量 (default: min(scenes, 10))
  --seed N               随机种子
